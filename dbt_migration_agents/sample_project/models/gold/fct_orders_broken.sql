-- Gold Layer: Orders Fact Table (BROKEN PATTERN)
-- Complexity: HIGH
-- ISSUE: Reads from intermediate chain instead of consolidated Silver
-- CORRECT: Read from stg_orders (consolidated) instead of int_orders_final_broken
...
-- This model reads from int_orders_final_broken, which is the end of
-- a 3-step intermediate chain: raw_orders -> int_cleaned -> int_enriched -> int_final
-- This creates unnecessary complexity with 4 hops to Gold.
--
-- MIGRATION GOAL: Consolidate the intermediate chain and read directly from
-- stg_orders, reducing the path to: raw_orders -> stg_orders -> fct_orders (2 hops)
--
-- After migration, this model should be replaced with fct_orders.sql
-- which reads from the consolidated stg_orders.sql

WITH orders AS (
    -- BROKEN: Reading from intermediate chain (4 hops from Bronze)
    -- CORRECT: Should read from stg_orders (2 hops from Bronze)
    SELECT *
