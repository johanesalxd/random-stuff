"""Run Dataplex scans that enrich BigQuery metadata for the lakehouse agent.

This is a *showcase* utility: it creates and runs Dataplex DataScan resources
(data profile + data documentation) over the Froyo Lakehouse agent's tables so
that BigQuery/Knowledge Catalog hold generated profiles, table/column
documentation, and dataset insights. It demonstrates that the metadata
components already exist -- wiring them into the agent's authored context is a
deliberate, separate step and is intentionally NOT done here.

Native vs federated behavior (see README "Metadata enrichment (showcase)"):

- Native BigQuery tables (``froyo_demo_ue4``): data profile + data
  documentation, published to Knowledge Catalog, plus the documentation results
  are published back to the table (labels) so column descriptions surface on the
  BigQuery Studio "Insights" tab and in the schema.
- AWS-federated Iceberg tables (``demo_glue_cat.<namespace>``): data profile
  only (best-effort; supported for Iceberg REST Catalog tables). Column
  descriptions CANNOT be written to their schema because the external Iceberg
  REST catalog is read-only to BigQuery DDL -- this is the cross-cloud frontier
  the showcase makes visible.

This script keeps the generic Dataplex REST plumbing byte-compatible with
``bq_caapi_ge/scripts/enrich_bigquery_metadata.py`` so the two stay easy to
sync. The lakehouse-specific parts are the config loading, the native/federated
split in ``main``, and ``publish_documentation_to_table`` (see the marked
"LAKEHOUSE-SPECIFIC" sections).

Usage::

    GOOGLE_API_USE_CLIENT_CERTIFICATE=false uv run python \
        scripts/enrich_bigquery_metadata.py --wait
...
# --- LAKEHOUSE-SPECIFIC: resolve config from ../config.local.env + .env --------
...
_CONFIG = load_lakehouse_config()
PROJECT_ID = _CONFIG.project_id
NATIVE_DATASET = _CONFIG.native_dataset
DATAPLEX_LOCATION = os.getenv("DATAPLEX_LOCATION", os.getenv("GCP_REGION", "us-east4"))
DATAPLEX_SCAN_PREFIX = os.getenv("DATAPLEX_SCAN_PREFIX", "froyo-agent")
# --- END LAKEHOUSE-SPECIFIC ----------------------------------------------------
...
def request(
    ...
    """Execute an authenticated REST request via curl.
    ...
    Raises:
        RuntimeError: If curl fails, JSON is invalid, or the API returns an error.
    """
    cmd = [
        "curl",
        "-s",
...
def bigquery_table_resource(project_id: str, dataset_id: str, table_id: str) -> str:
    ...
    Args:
        project_id: Google Cloud project ID.
        dataset_id: BigQuery dataset ID (or ``catalog.namespace`` for federated).
...
def build_data_profile_payload(
    ...
    Args:
        ...
        export_results_table: Optional BigQuery table resource for exported
            profile results.
...
def upsert_scan(
    ...
    try:
        ...
        if "already exists" in str(e).lower() or "already_exists" in str(e).lower():
            ...
            query = f"updateMask={update_mask_for_payload(payload)}"
            operation = request(
                "PATCH",
...
def documentation_labels(scan_name: str) -> dict[str, str]:
    ...
    the BigQuery Studio "Insights" tab and in the table schema.
...
def publish_documentation_to_table(
    ...
    """Publish documentation scan results back to a native BigQuery table.

    This patches the table's labels so the generated descriptions surface in
    BigQuery. It is only valid for native tables; federated Iceberg REST catalog
    tables reject schema/metadata writes via BigQuery DDL.

    Args:
        dataset_id: Native BigQuery dataset ID.
        table_id: BigQuery table ID.
        scan_name: The data documentation scan ID whose results to publish.
        token: Bearer token.
        dry_run: Whether to skip the API call.
...
def apply_documentation_to_schema(
    ...
    so descriptions appear in the table's schema (native tables only; federated
    Iceberg REST catalog schemas reject DDL writes).
...
def log_view_links(native_tables: list[str]) -> None:
    """Log where to view the generated metadata for the enriched tables.

    Args:
        native_tables: Native table ids that were enriched.
    """
    logger.info("View generated metadata:")
    for table_id in native_tables:
        logger.info(
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run Dataplex profile and documentation scans for the "
        "Froyo Lakehouse agent's tables (showcase)."
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Native BigQuery tables to enrich. Defaults to all native agent tables.",
    )
    parser.add_argument(
        "--generation-scope",
        choices=(
    ...
    parser.add_argument(
        "--skip-dataset-docs",
        action="store_true",
        help="Skip the dataset-level data documentation scan.",
    )
    parser.add_argument(
        "--skip-federated",
        action="store_true",
        help="Skip the best-effort data profile scan on AWS-federated tables.",
    )
    parser.add_argument(
        "--no-publish",
        action="store_true",
        help="Do not publish scan results to Knowledge Catalog or to the table.",
    )
    parser.add_argument(
        "--wait",
        action="store_true",
...
def enrich_native_table(
    ...
    """Run profile + documentation scans for one native table and publish them.

    Args:
        table_id: Native BigQuery table ID.
    ...
    """
    table_resource = bigquery_table_resource(PROJECT_ID, NATIVE_DATASET, table_id)
    if not args.skip_profile:
        name = scan_id("profile", NATIVE_DATASET, table_id)
    ...
    if not args.skip_table_docs:
        name = scan_id("table-docs", NATIVE_DATASET, table_id)
        ...
        if publish:
            # Publish to Knowledge Catalog / Insights (labels) ...
            publish_documentation_to_table(
                NATIVE_DATASET, table_id, name, token, args.dry_run
            )
            ...
            apply_documentation_to_schema(
                NATIVE_DATASET, table_id, overview, descriptions, token, args.dry_run
            )


def profile_federated_table(
    dataset_id: str,
    table_id: str,
    args: argparse.Namespace,
    token: str,
    ...
    """Best-effort data profile for one AWS-federated Iceberg table.

    Data documentation and schema-level column descriptions are not attempted:
    the external Iceberg REST catalog schema is read-only to BigQuery DDL. A
    profile failure is logged and does not abort the run.

    Args:
        dataset_id: Federated ``catalog.namespace`` dataset ID.
        table_id: Iceberg table ID.
        args: Parsed CLI arguments.
        token: Bearer token.
        sampling_percent: Resolved sampling percentage (STANDARD mode).
        publish: Whether to publish results to Knowledge Catalog.
    """
    ref = f"{dataset_id}.{table_id}"
    if args.skip_profile:
        return
    table_resource = bigquery_table_resource(PROJECT_ID, dataset_id, table_id)
    name = scan_id("profile-fed", dataset_id, table_id)
    payload = build_data_profile_payload(
        resource=table_resource,
        mode=args.profile_mode,
        publish=publish,
    ...
    try:
        ...
        logger.warning(
            "Federated data profile for %s was not created (%s). Profiling of "
            "AWS-federated Iceberg tables may require console access in your "
            "project; continuing.",
            ref,
            e,
        )
    logger.info(
        "Federated table %s: column descriptions are NOT written to the schema "
        "(external Iceberg REST catalog schemas are read-only to BigQuery DDL).",
        ref,
    )


def main() -> None:
    """Create and run Dataplex metadata enrichment scans for the agent."""
    if not PROJECT_ID:
    ...
    native_tables = (
        list(args.tables) if args.tables else list(native_table_ids(_CONFIG))
    )
    federated = () if args.skip_federated else federated_table_refs(_CONFIG)

    logger.info(
        "Enriching %d native + %d federated tables (project=%s, location=%s).",
        len(native_tables),
        len(federated),
        PROJECT_ID,
        DATAPLEX_LOCATION,
    ...
    if not args.skip_dataset_docs:
        name = scan_id("dataset-docs", NATIVE_DATASET)
        payload = build_data_documentation_payload(
            resource=bigquery_dataset_resource(PROJECT_ID, NATIVE_DATASET),
    ...
    for dataset_id, table_id in federated:
        profile_federated_table(
            dataset_id, table_id, args, token, sampling_percent, publish
        )

    if not args.dry_run:
        log_view_links(native_tables)
