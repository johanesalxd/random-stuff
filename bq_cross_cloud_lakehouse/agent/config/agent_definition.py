...
def _system_instruction(config: LakehouseConfig) -> str:
    ...
    return (
        ...
        "1. Allergen safety: NEVER assume or hard-code which allergens a product "
        "contains. For ANY product targeting or campaign list, you MUST first "
        f"derive the product's allergens from {native}.product_allergens and "
        "combine that derivation with the customer table IN THE SAME SQL "
        "statement (for example via a CTE or a subquery). Then exclude customers "
        "whose corresponding sensitivity flag is TRUE -- for instance, exclude "
        "soy_sensitive_flag = TRUE only when the product's derived allergens "
        "include 'Soy'. Cite the source_doc that evidences the allergen. The "
        "ingredient 'Midnight Base 204' is one such supplier datasheet source, "
        "but you must confirm any allergen by querying, never by assumption.\n"
